...
PosOnlyFunctor::PosOnlyFunctor(const Matrix3X& data_points, const MeshTopology& mesh, const DataConstraints& constraints)
	: BaseFunctor(mesh.num_vertices * 3 + data_points.cols() * 2 + 9,   /* number of parameters */
		data_points.cols() * 3 + constraints.size() * 3 + data_points.cols(), /* number of residuals */
		data_points.cols() * 3 + data_points.cols() * 6 + data_points.cols() * 15 + constraints.size() * 3 + constraints.size() * 15 + data_points.cols() * 4,	/* number of Jacobian nonzeros */
		data_points,
		mesh,
		constraints) {
