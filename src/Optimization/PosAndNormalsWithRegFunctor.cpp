...
PosAndNormalsWithRegFunctor::PosAndNormalsWithRegFunctor(const Matrix3X& data_points, const Matrix3X &data_normals, const MeshTopology& mesh)
	: BaseFunctor(mesh.num_vertices * 3 + data_points.cols() * 2,   /* number of parameters */
		...
		data_points.cols() * 3 + data_points.cols() * 6 + data_points.cols() * 6 + data_points.cols() * 9 + mesh.num_vertices * 3,	/* number of Jacobian nonzeros */
		data_points,
		mesh),
...
void PosAndNormalsWithRegFunctor::df_impl(const InputType& x, Eigen::TripletArray<Scalar, typename JacobianType::Index>& jvals) {
	...
	this->dE_thinplate_d_X(x, x.rigidTransf, jvals, X_base, data_points.cols() * 3 + data_normals.cols() * 3);
}
