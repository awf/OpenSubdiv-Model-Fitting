...
PosAndNormalsWithRegFunctor::PosAndNormalsWithRegFunctor(const Matrix3X& data_points, const Matrix3X &data_normals, const MeshTopology& mesh)
	: BaseFunctor(mesh.num_vertices * 3 + data_points.cols() * 2 + 9,   /* number of parameters */
		data_points.cols() * 3 + data_normals.cols() * 3 + mesh.num_vertices * 3, /* number of residuals */
		data_points.cols() * 3 + data_points.cols() * 6 + data_points.cols() * 6 + data_points.cols() * 9 + mesh.num_vertices * 3 + data_points.cols() * 15,	/* number of Jacobian nonzeros */
		data_points,
		mesh),
...
void PosAndNormalsWithRegFunctor::f_impl(const InputType& x, ValueType& fvec) {
	...
	this->E_thinplate(x, fvec, data_points.cols() * 3 + data_normals.cols() * 3);
}
...
void PosAndNormalsWithRegFunctor::df_impl(const InputType& x, Eigen::TripletArray<Scalar, typename JacobianType::Index>& jvals) {
	...
	this->dE_thinplate_d_X(x, jvals, X_base, data_points.cols() * 3 + data_normals.cols() * 3);
}
