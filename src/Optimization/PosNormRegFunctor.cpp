#include "PosNormRegFunctor.h"

#include <iostream>

PosNormRegFunctor::PosNormRegFunctor(const Matrix3X& data_points, const Matrix3X &data_normals, const MeshTopology& mesh)
	: BaseFunctor(mesh.num_vertices * 3 + data_points.cols() * 2 + 3 + 1,   /* number of parameters */
		data_points.cols() * 3 + data_normals.cols() * 3 + 1, /* number of residuals */
		data_points,
		mesh),
...
int PosNormRegFunctor::operator()(const InputType& x, ValueType& fvec) {
	...
	float tpLambda = 1e-2;
	for (int i = 0; i < data_points.cols(); i++) {
		// Add the smoothenss term (Thin plate energy)	
		fvec(tp_base) += tpLambda * (pow(dSduu.col(i).norm(), 2) + 2 * pow(dSduv.col(i).norm(), 2) + pow(dSdvv.col(i).norm(), 2));
	}
...
int PosNormRegFunctor::df(const InputType& x, JacobianType& fjac) {
	...
	Index tp_base = data_points.cols() * 3 + data_normals.cols() * 3;
...
void PosNormRegFunctor::increment_in_place(InputType* x, StepType const& p) {
	Index nPoints = data_points.cols();
	Index X_base = nPoints * 2;
	Index ubase = 0;
	Index t_base = X_base + x->nVertices() * 3;
...
void PosNormRegFunctor::initQRSolver(SchurlikeQRSolver &qr) {
	// set block size
