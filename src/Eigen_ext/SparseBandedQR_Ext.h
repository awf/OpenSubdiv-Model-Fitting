...
void householder_product_transposed(const MatrixXd &H, const VectorXd &_hCoeffs, MatrixXd &v) {
	for (int j = 0; j < v.cols(); j++) {
		for (int k = 0; k < H.cols(); k++) {
			v.col(j) -= (H.col(k).dot(v.col(j)) * _hCoeffs(k)) * H.col(k);
		}
	}
}
...
void SparseBandedQR_Ext<MatrixType, OrderingType>::factorize(const MatrixType& mat)
{
	...
	#define NNZ_ROWS (m_blockRows * 12)	// Yields error in norm in range of 1e-5
	for (Index i = 0; i < numBlocks; i++) {
		...
		if (blockRows > NNZ_ROWS) {
			houseqr.compute(Ji.bottomRows(NNZ_ROWS));
		...
		MatrixXd H = MatrixXd::Zero(blockRows, blockCols);
		for(int bc = 0; bc < blockCols; bc++) {
			H(bc, bc) = 1.0;
			H.block(bc + 1 + numZeros, bc, houseqr.householderQ().essentialVector(bc).rows(), 1) = houseqr.householderQ().essentialVector(bc);

			m_hcoeffs(bs + bc) = houseqr.hCoeffs()(bc);
		}
		for (int r = 0; r < blockRows; r++) {
			for (int bc = 0; bc < blockCols; bc++) {
				Qvals.add_if_nonzero(bs + r, bs + bc, H(r, bc));
			}
		}
		...
		// For any other block
			...
			// Multiplication by the householder vectors
			MatrixXd Vleft = Ji;
			MatrixXd Vright = mat.block(bs, bs + blockCols, blockRows, blockCols).toDense();
			householder_product_transposed(H, houseqr.hCoeffs(), Vleft);
			householder_product_transposed(H, houseqr.hCoeffs(), Vright);

			tmp = DenseMatrixType::Zero(blockRows, blockCols * 2);
